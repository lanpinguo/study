...
class Forwarder(ZeroconfService):
    ...
    def handle_serial_read(self):
        ...
        try:
            ...
            self.log.info("Serial IN: {}".format(data))
            if data:
                # store data in buffer if there is a client connected
                if self.socket is not None:
                    # escape outgoing data when needed (Telnet IAC (0xff) character)
                    if self.rfc2217:
                        data = b"".join(self.rfc2217.escape(data))
                    self.buffer_ser2net.extend(data)
            else:
                self.handle_serial_error()
        except Exception as msg:
            raise
            self.log.error("handle_serial_read: {}".format(msg))
            self.handle_serial_error(msg)
    ...
    def handle_serial_write(self):
        ...
        try:
            # write a chunk
            data = bytes(self.buffer_net2ser)
            self.log.info("Serial OUT: {}".format(data))
            n = os.write(self.serial.fileno(), data)
            # and see how large that chunk was, remove that from buffer
            self.buffer_net2ser = self.buffer_net2ser[n:]
        except Exception as msg:
            self.log.error("handle_serial_write: {}".format(msg))
            self.handle_serial_error(msg)

    def handle_serial_error(self, error=None):
        """Serial port error"""
        self.log.error("Serial port error: {}".format(error))
    ...
    def handle_socket_read(self):
        ...
        try:
            ...
            self.log.info("SOCK IN: {}".format(data))
            if data:
    ...
    def handle_socket_write(self):
        ...
        try:
            # write a chunk
            data = bytes(self.buffer_ser2net)
            self.log.info("SOCK OUT: {}".format(data))
    ...
    def handle_server_error(self):
        """Socket server fails"""
        self.log.error("Socket server fails")
        self.close()
